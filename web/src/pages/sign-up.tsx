...
export default function SignUp() {
  ...
  const nextStep = () => {
    ...
    if (passwords[0] !== passwords[1]) {
      setDiffPassword('Please make sure your password is the same in both fields.');
      return;
    }
    setIsContinue(true);
  }
  ...
  const createWallet = async () => {
    if (addresses[0].length === 0) {
      setNoRecovAddr('Please enter atleast one recovery address.');
      return;
    }
    setIsLoading(true);
    const res = await fetch('/api/getSimpleAccount');
        ...
        ? (<>
          ...
          <div className="space-y-2 pb-4 pt-8">
            <div>
              ...
              <input required type="password" value={passwords[0]} onChange={(e) => setPasswords([e.target.value, passwords[1]])} className="input"/>
            </div>
            <div>
              <label className="block mb-2">Confirm your password</label>
              <input required type="password" value={passwords[1]} onChange={(e) => setPasswords([passwords[0], e.target.value])} className="input"/>
            </div>
          </div>
          ...
          <div className="space-y-2 pb-4 pt-8">
            <div>
              ...
              <input 
                ...
                className="input" 
                value={addresses[0]}
                onChange={(e) => setAddresses([e.target.value, addresses[1], addresses[2]])}
            ...
            <div>
              ...
              <input 
                className="input"
                value={addresses[1]}
                onChange={(e) => setAddresses([addresses[0], e.target.value, addresses[2]])}
                placeholder="0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045"
              />
            </div>
            <div>
              ...
              <input 
                className="input"
                value={addresses[2]}
                onChange={(e) => setAddresses([addresses[0], addresses[1], e.target.value])}
                placeholder="0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045"
              />
            </div>
          </div>
